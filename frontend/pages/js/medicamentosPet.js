document.addEventListener('DOMContentLoaded', () => {
    ...
    const classificarStatus = (item) => {
        ...
        if (item.data && new Date(item.data) <= hoje) {
            return { status: 'Aplicadas', classeStatus: 'status-aplicada', classeIcone: 'icon-aplicada', icone: 'fa-check', textoData: `Administrado em: ${item.data.split('T')[0]}` };
        } 
        
        if (item.data_prevista) {
            const dataPrevista = new Date(item.data_prevista);
            if (dataPrevista < hoje) {
                return { status: 'Atrasadas', classeStatus: 'status-atrasada', classeIcone: 'icon-atrasada', icone: 'fa-triangle-exclamation', textoData: `Atrasado desde: ${item.data_prevista.split('T')[0]}` };
            } else {
                return { status: 'Futuras', classeStatus: 'status-futura', classeIcone: 'icon-futura', icone: 'fa-calendar-days', textoData: `Próxima dose: ${item.data_prevista.split('T')[0]}` };
            }
        }
    ...
    const renderizarLista = (filtro) => {
        ...
        const dadosFiltrados = dadosMedicamentos.filter(item => filtro === 'Tudo' || item.infoStatus.status === filtro);
            ...
            const cardHtml = `
                <div class="list-item">
                    ...
                    <div class="item-details">
                        ...
                        <span class="${item.infoStatus.classeStatus}"><i class="fa-solid ${item.infoStatus.icone}"></i> ${item.infoStatus.status === 'Aplicadas' ? 'Aplicado' : item.infoStatus.status === 'Atrasadas' ? 'Atrasado' : 'Futuro'}</span>
                    </div>
                </div>
            `;
    ...
    const buscarMedicamentos = async () => {
        ...
        try {
            ...
            // Puxa apenas a categoria "Medicação" da sua API
            const apenasMedicamentos = historicoGeral.filter(i => i.categoria === 'Medicação');
            
            dadosMedicamentos = apenasMedicamentos.map(item => ({
                ...item,
